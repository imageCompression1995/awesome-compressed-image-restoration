...
|[JPEG Artifacts Removal via Compression Quality Ranker-Guided Networks](https://www.ijcai.org/proceedings/2020/0079.pdf)  | 2020 | IJCAI| |Existing CNN\-based methods only consider pixel\-level loss functions to guide network training. JPEG compression factor has not been fully utilized, and is also hard to use due to the non-differentiability.| They first learn a CNN\-based quality ranker to measure the compression degree, and then propose one quality related loss and one feature matching loss to guide de\-blocking and perceptual quality optimization. Noted that they use dilated convolutions to extract multi-scale features.|
